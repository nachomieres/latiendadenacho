...
function increaseQuantity(link) {
		...
		if (newQuantity <= 5) {
			...
			showWarningModal('La cantidad maxima es 5');
		}	
}
...
function updateQuantity(productId, quantity) {
	...
	$.ajax({
		...
		showErrorModal("Error actualizando la cantidad del producto.");
	});	
}
...
function removeProduct(link) {
	...
	$.ajax({
		...
		showModalDialog("Carrtito", response);
		
	}).fail(function() {
		showErrorModal("Error eliminado el producto");
	});				
}
