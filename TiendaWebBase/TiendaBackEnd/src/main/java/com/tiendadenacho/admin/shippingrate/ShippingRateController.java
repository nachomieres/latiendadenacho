...
public class ShippingRateController {
	...
	public String newRate(Model model) {
		...
		model.addAttribute("pageTitle", "Nueva tarifa de envio");
	...
	public String saveRate(ShippingRate rate, RedirectAttributes ra) {
		try {
			service.save(rate);
			ra.addFlashAttribute("message", "La tarifa de envio se ha guardado correctamente.");
	...
	public String updateCODSupport(@PathVariable(name = "id") Integer id,
		...
		try {
			...
			ra.addFlashAttribute("message", "Contrarreembolso para la tarifa de envio con ID " + id + " ha sido actualizada.");
		} catch (ShippingRateNotFoundException ex) {
			ra.addFlashAttribute("message", ex.getMessage());
		}
		return defaultRedirectURL;
	...
	public String deleteRate(@PathVariable(name = "id") Integer id,
		...
		try {
			service.delete(id);
			ra.addFlashAttribute("message", "La tarifa de envio con ID " + id + " has been deleted.");
		} catch (ShippingRateNotFoundException ex) {
			ra.addFlashAttribute("message", ex.getMessage());
		}
		return defaultRedirectURL;
